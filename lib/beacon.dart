...
abstract class Eddystone extends Beacon {
  const Eddystone({required this.frameType, required int tx, required ScanResult scanResult}) : super(tx: tx, scanResult: scanResult);
...
class EddystoneUID extends Eddystone {
  ...
  const EddystoneUID({required int frameType, required this.namespaceId, required this.beaconId, required int tx, required ScanResult scanResult}) : super(tx: tx, scanResult: scanResult, frameType: frameType);
  ...
  int get hash => hashObjects(["EddystoneUID", EddystoneServiceId, this.frameType, this.namespaceId, this.beaconId, this.tx]);
}

class EddystoneEID extends Eddystone {
  final String ephemeralId;

  const EddystoneEID({required int frameType, required this.ephemeralId, required int tx, required ScanResult scanResult}) : super(tx: tx, scanResult: scanResult, frameType: frameType);
  ...
  int get hash => hashObjects(["EddystoneEID", EddystoneServiceId, this.frameType, this.ephemeralId, this.tx]);
}
...
class IBeacon extends Beacon {
  ...
  const IBeacon({required this.uuid, required this.major, required this.minor, required int tx, required ScanResult scanResult}) : super(tx: tx, scanResult: scanResult);
  ...
  int get hash => hashObjects(["IBeacon", IBeaconManufacturerId, this.uuid, this.major, this.minor, this.tx]);
}
